  ...
  return (
    <section className={styles.preview} aria-labelledby="future-bloom-title">
      <div className={styles.heading}>
        <div><span>미래 정원</span><h2 id="future-bloom-title" ref={titleRef} tabIndex={-1}>이 씨앗은 어디까지 자랄까요?</h2></div>
        <button type="button" onClick={onClose} aria-label="미래 정원 미리보기 닫기">닫기</button>
      </div>
      <div className={styles.content}>
        <div className={styles.visual}><LazyGardenScene plan={plan} quality={quality} reducedMotion={reducedMotion} label={`${plan.stage.day}일째 성장 가능성`} /></div>
        <div className={styles.details}>
          <div className={styles.scrubber} role="group" aria-label="성장 단계">
            {plans.map((item, index) => <button key={item.stage.day} type="button" aria-pressed={selectedIndex === index} onClick={() => setSelectedIndex(index)}>{item.stage.day}일째</button>)}
          </div>
