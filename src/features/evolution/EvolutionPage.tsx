...
export function EvolutionPage() {
  ...
  return (
    <div className={layout.page}>
      <PageIntro eyebrow="진화 온실" title="한 씨앗이 만날 다섯 계절"><p>문서처럼 읽는 대신, 같은 마음씨앗이 단계마다 어떻게 가지와 빛을 더하는지 직접 살펴보세요.</p></PageIntro>
      <section className={styles.conservatory}>
        <div className={styles.glassLines} aria-hidden="true" />
        <div className={styles.stageVisual}>
          <LazyGardenScene plan={selected} quality={state.settings.renderQuality} reducedMotion={state.settings.reducedMotion} interactive label={`${selected.stage.day}일째 ${selected.stage.name} 회전 미리보기`} />
          <span className={styles.mode}>{isDemo ? "고정 데모 씨앗" : "내 마음씨앗의 무상태 미리보기"}</span>
        </div>
        <div className={styles.stageCopy}>
          <p>{selected.stage.day}일째</p>
          <h2>{selected.stage.name}</h2>
          <strong>{selected.stage.description}</strong>
          <span>{selected.stage.unlock}</span>
          <p className={styles.notice}>{isDemo ? "아직 실제 기록이 없어 데모 씨앗을 보여줍니다. 오늘의 문장을 남기면 내 마음씨앗으로 바뀝니다." : "이 화면은 가능성을 살펴보는 미리보기이며, 실제 기록 수와 해금 상태를 변경하지 않습니다."}</p>
        </div>
        <div className={styles.shelf} role="group" aria-label="진화 단계 선택">
          {plans.map((plan, index) => (
            <button key={plan.stage.day} type="button" aria-pressed={selectedIndex === index} onClick={() => setSelectedIndex(index)}>
              <div><StaticPlant plan={plan} label={`${plan.stage.day}일째 단계`} /></div>
              <span>{plan.stage.day}일째</span><strong>{plan.stage.name}</strong>
            </button>
          ))}
        </div>
      </section>
      <section className={styles.legend}>
        <article><span>01</span><h3>마음씨앗</h3><p>첫 문장에서 태어나 누적 고유 기록일과 함께 자라는 정원의 중심 식물입니다.</p></article>
        <article><span>02</span><h3>매일의 표본</h3><p>그날의 문장과 인상을 반영해 매일 하나씩 정원 주변에 놓이는 표본입니다.</p></article>
