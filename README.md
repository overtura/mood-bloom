...
3. TypeScript 생성 엔진이 문장과 날짜에서 결정적인 seed를 만들고 3D 식물을 피웁니다.
4. Future Bloom에서 Day 1·3·7·14·30의 성장 가능성을 살펴봅니다.
...
- WebGPU를 우선하고 사용할 수 없으면 WASM을 사용합니다. 모델 준비가 실패하거나 오래 걸리면 결정적인 keyword/rule fallback으로 흐름을 계속합니다.
...
`speciesSeed`는 식물의 기본 품종과 정체성을, `dailySeed`는 해당 날짜의 가지·꽃잎·장식 배치를 결정합니다. 모든 무작위 값은 seed 기반 PRNG를 통하며 렌더링 도중 제품 상태를 변경하지 않습니다.
